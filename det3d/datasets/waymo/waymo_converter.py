...
def convert(idx):
    ...
    for frame_id, data in enumerate(dataset):
        ...
        lidar_path = os.path.join(LIDAR_PATH, 'seq_{}_frame_{}.pkl'.format(idx, frame_id))
        if not os.path.exists(lidar_path):
            with open(lidar_path, 'wb') as f:
                pickle.dump(decoded_frame, f)
                
        annos_path = os.path.join(ANNO_PATH, 'seq_{}_frame_{}.pkl'.format(idx, frame_id))
        if not os.path.exists(annos_path):
            with open(annos_path, 'wb') as f:
